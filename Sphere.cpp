...
BoundingVolume
Sphere::calcBoundingVolume() {
	Vector3 min = m_center - m_radius;
	Vector3 max = m_center + m_radius;
	return BoundingVolume(min, max);
}

Vector3
Sphere::getGeometriNormal(Vector3 point) {
	Vector3 Vop = point - m_center;
	Vop.normalize();
	return Vop;
}

void Sphere::Move(Vector3 dist) {
	setCenter(m_center + dist);
}
